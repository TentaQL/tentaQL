...
db.getConstraints = async (req, res, next) => {
  ...
  for (let i = 0; i < constraintData.length; i++) {
    if (requiredTables[constraintData[i].table_name] === undefined) {
      requiredTables[constraintData[i].table_name] = [
        [constraintData[i].column_name, constraintData[i].is_nullable]
      ];
    } else {
      requiredTables[constraintData[i].table_name].push([
        constraintData[i].column_name,
        constraintData[i].is_nullable
      ]);
    }
  }
